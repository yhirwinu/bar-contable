...
// Configuración de la base de datos SQLite (se guarda en un archivo local database.sqlite)
...
function initDb() {
  db.serialize(() => {
    ...
    // Valores iniciales por defecto si están vacíos
...
// ENDPOINTS DE LA API (Sincronización en vivo)

// Obtener todo el estado del sistema
  ...
  db.get("SELECT value FROM config WHERE key = 'tasa'", (err, row) => {
    ...
    db.all("SELECT * FROM inventario", (err, rows) => {
      ...
      db.all("SELECT * FROM cuentasHoy", (err, rows) => {
        // Parsear pedidos almacenados como JSON string
...
// Guardar/Actualizar todo el estado o sincronizar acciones
